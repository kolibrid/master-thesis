...
Max number of edges to draw in the blood dataset is 16070 for the node ARGLU1.
...
%     \draw plot[mark=star*,xshift=-6cm] file {data/bloodScatter.data};
%     \draw[->,xshift=-6cm] (6,0) -- coordinate (x axis mid) (17,0);
%     \draw[->,xshift=-6cm] (6,0) -- coordinate (y axis mid)(6,27);
%     \foreach \x in {0,2300,4600,6900,9200,11500,13800,16100}
%         \draw [xshift=-6cm](\x cm,1pt) -- (\x cm,-3pt)
%             node[anchor=north] {$\x$};
%     \foreach \y/\ytext in {0,20,40,60,80,100,120,140}
%         \draw (1pt,\y cm) -- (-3pt,\y cm) node[anchor=east] {$\ytext$};
%     \node[below=1cm] at (x axis mid) {Number of edges};
%     \node[rotate=90, above=2cm] at (y axis mid) {Number of nodes};
% \end{tikzpicture}
