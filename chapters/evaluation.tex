...
Number of nodes & 1\% average low (ms) & 0.25\% average low (ms) \\
size/3 & sth & sth \\
size/2 & sth & sth \\
size & 8.442 & 14.774 \\
\end{tabular}
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    ...
    \addplot[scatter,only marks] file{data/scalabilityBlood.dat};
    \end{axis}
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    ...
    \addplot[scatter,only marks] file{data/scalabilityBiopsy.dat};
    \end{axis}
...
\begin{figure}[h!]
    ...
    \centering
    \includegraphics[width=\textwidth]{setParent}
    \caption{Bottleneck when setting the LineRender object as a child of the network.}
    \label{fig:bignet_vr}
\end{figure}
