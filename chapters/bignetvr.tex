...
In Figure \ref{fig:oculus_quest_inputs} we can see which actions correspond to each input from the controllers. The actions are the following:
\begin{enumerate}
  \item Snap rotation: It allows the user to instantly rotate to the right or to the left 45$^{\circ}$.
  \item Filter menu: The user can filter the nodes of the network according to the filtering algorithm used in BigNet VR.
  \item Scale environment: The network can be scaled or "zoomed".
  \item Translate environment: The network can be translated or moved to other positions in the VR environment.
  \item Pointer: TODO This needs to be removed since we are not using it in BigNet VR.
  \item Select item in menu: It allows us interact with the menu, for instance enable or disable the checkboxes or drag the slider.
  \item Oculus menu: It opens the menu from oculus and pauses BigNet VR.
  \item Teleport: It teleports the user to another position on the floor of the VR envirnoment.
\end{enumerate}

We will explain now in the following subsections, how we implemented the system and the different methods in more detail that we have used for the data visualization.
