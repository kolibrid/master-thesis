...
The following diagram in Figure \ref{fig:create_network} explains in a broad way what steps we follow to build the network in Unity from the two source files. We start with the 2 files described before, containing the data for the network. We process the file with information about the categories and build a particle system object in Unity, where each particle corresponds to a node. We also add a color to each particle, which is related to the category that the particle belongs to. Finally we apply a cluster algorithm using the data from the other file with information about the relationships.
